...
public interface PlayerHandMapper {
    ...
    @Select("SELECT cards.id, cardAttribute, cardStrong FROM  playerhand join cards on (card_id = cards.id) WHERE userName = #{userName} and  (cards.id >= 11 and cards.id<=16);")
